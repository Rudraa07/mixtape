...
var AUTH_HTML = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"><title>Mixtape</title><style>@import url(\'https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;700&family=JetBrains+Mono:wght@400;500&display=swap\');:root{--bg:#1a1816;--panel:#221f1c;--amber:#e8a33d;--teal:#4a8c8c;--cream:#f2ede4;--red:#c1443c;--line:#3a352f;--muted:#9a9186;}*{box-sizing:border-box;}body{background:var(--bg);color:var(--cream);font-family:\'Space Grotesk\',sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;padding:20px;}.card{background:var(--panel);border:1px solid var(--line);border-radius:18px;padding:28px 24px;width:100%;max-width:360px;}h1{font-size:24px;margin:0 0 4px;}.sub{font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--muted);letter-spacing:1px;margin-bottom:24px;}.tabs{display:flex;gap:8px;margin-bottom:20px;}.tab{flex:1;padding:9px;border-radius:8px;border:1px solid var(--line);background:transparent;color:var(--muted);font-family:\'JetBrains Mono\',monospace;font-size:12px;cursor:pointer;}.tab.active{background:var(--amber);border-color:var(--amber);color:var(--bg);font-weight:700;}input{display:block;width:100%;background:var(--bg);border:1px solid var(--line);border-radius:8px;padding:10px 12px;color:var(--cream);font-family:\'JetBrains Mono\',monospace;font-size:13px;margin-bottom:10px;}.btn{display:block;width:100%;padding:12px;border-radius:8px;border:none;background:var(--amber);color:var(--bg);font-family:\'JetBrains Mono\',monospace;font-size:13px;font-weight:700;cursor:pointer;margin-top:4px;}.msg{font-family:\'JetBrains Mono\',monospace;font-size:12px;margin-top:12px;text-align:center;min-height:18px;}.err{color:var(--red);}.ok{color:var(--teal);}.form{display:none;}.form.active{display:block;}</style></head><body><div class="card"><h1>Mixtape</h1><div class="sub">PRIVATE ACCESS</div><div class="tabs"><button class="tab active" onclick="show(\'login\')">Login</button><button class="tab" onclick="show(\'signup\')">Sign up</button></div><div class="form active" id="loginForm"><input type="email" id="loginEmail" placeholder="Email"><input type="text" id="loginCode" placeholder="Access code" style="text-transform:uppercase" maxlength="8"><button class="btn" onclick="doLogin()">Enter</button><div class="msg" id="loginMsg"></div></div><div class="form" id="signupForm"><input type="text" id="signupName" placeholder="Your name"><input type="email" id="signupEmail" placeholder="Email"><button class="btn" onclick="doSignup()">Request access</button><div class="msg" id="signupMsg"></div></div></div><script>' + AUTH_SCRIPT + '<\/script></body></html>';
...
async function handleRequest(request, env) {
    ...
    if (path === "/api/upload" && method === "POST") {
      ...
      if (user.storage_limit_mb !== -1 && (user.storage_used_mb || 0) + fileSizeMb > user.storage_limit_mb) {
