...
function adminHTML(users) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Mixtape Admin</title><style>' +
    ...
    '.pending{color:#e8a33d;}.approved{color:#4a8c8c;}' +
    'button{background:#4a8c8c;border:none;color:#1a1816;padding:5px 10px;cursor:pointer;border-radius:4px;font-family:monospace;margin-right:4px;margin-top:4px;}' +
  ...
  async fetch(request, env) {
    ...
    if (path === '/admin/login') {
      ...
      if (method === 'POST') {
        ...
        return new Response('', {
          status: 302,
          headers: { 'Location': '/admin', 'Set-Cookie': 'mx_admin=authed; Path=/; HttpOnly; SameSite=Strict; Max-Age=86400' }
        });
      }
    }
    ...
    if (path === '/api/login' && method === 'POST') {
      ...
      return new Response(JSON.stringify({ ok: true }), {
        status: 200,
        headers: { 'Content-Type': 'application/json', 'Set-Cookie': 'mx_session=' + token + '; Path=/; HttpOnly; SameSite=Strict; Max-Age=2592000' }
      });
    }
